...
public class Extra_french2 extends Activity implements OnInitListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_extra_french2);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        case REQ_CODE_SPEECH_INPUT: {
            if (resultCode == RESULT_OK && null != data) {
                ...
                {
                	Intent inn=new Intent(getApplicationContext(),Extra_french8.class);
                	inn.putExtra("Username", username);
                	startActivity(inn);
                }
                else if(voice.equalsIgnoreCase("next"))
                {
                	Intent inn=new Intent(getApplicationContext(),Extra_french9.class);
                	inn.putExtra("Username", username);
                	startActivity(inn);
                }
                else
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.extra_french2, menu);
		return true;
	}
	...
	private void speakOut() {
		...
		tts.speak(" ù two three four five six ",TextToSpeech.QUEUE_FLUSH, null);
