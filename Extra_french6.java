...
public class Extra_french6 extends Activity implements OnInitListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_extra_french6);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        case REQ_CODE_SPEECH_INPUT: {
            if (resultCode == RESULT_OK && null != data) {
                ...
                {
                	Intent inn=new Intent(getApplicationContext(),Extra_french1.class);
                	inn.putExtra("Username", username);
                	startActivity(inn);
                }
                else if(voice.equalsIgnoreCase("next"))
                {
                	Intent inn=new Intent(getApplicationContext(),Extra_french7.class);
                	inn.putExtra("Username", username);
                	startActivity(inn);
                }
                else
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.extra_french6, menu);
		return true;
	}
	...
	private void speakOut() {
		...
		tts.speak("   ï  one two four five six  ",TextToSpeech.QUEUE_FLUSH, null);
